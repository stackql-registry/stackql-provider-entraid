...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.groups.owners" /></td></tr>
</tbody></table>
...
<tr>
    ...
    <td>Retrieve a list of the group's owners. The owners are a set of users or service principals who are allowed to modify the group object. Owners are currently not available in Microsoft Graph for groups that were created in Exchange, distribution groups, or groups that are synchronized from an on-premises environment.</td>
</tr>
<tr>
    ...
    <td><a href="#parameter-group-id"><code>group-id</code></a></td>
    <td></td>
    <td>Add a user or service principal to a Microsoft 365 or security group's owners. The owners are a set of users or service principals who are allowed to modify the group object.</td>
</tr>
<tr>
    ...
    <td>Remove an owner from a Microsoft 365 group or a security group through the owners navigation property. Once owners are assigned to a group, the last owner (a user object) of the group cannot be removed.</td>
</tr>
<tr>
...
```sql
INSERT INTO entraid.groups.owners (
@odata.id,
group-id
)
SELECT 
...
```sql
DELETE FROM entraid.groups.owners
WHERE group-id = '{{ group-id }}' --required
