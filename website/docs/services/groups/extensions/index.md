...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.groups.extensions" /></td></tr>
</tbody></table>
...
id,
@odata.type
FROM entraid.groups.extensions
...
```sql
INSERT INTO entraid.groups.extensions (
id,
@odata.type,
group-id
)
...
```sql
UPDATE entraid.groups.extensions
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.groups.extensions
WHERE group-id = '{{ group-id }}' --required
AND extension-id = '{{ extension-id }}' --required
