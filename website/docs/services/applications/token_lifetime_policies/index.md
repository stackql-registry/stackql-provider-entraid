...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.applications.token_lifetime_policies" /></td></tr>
</tbody></table>
...
FROM entraid.applications.token_lifetime_policies
...
```sql
INSERT INTO entraid.applications.token_lifetime_policies (
@odata.id,
application-id
)
SELECT 
...
```sql
DELETE FROM entraid.applications.token_lifetime_policies
WHERE application-id = '{{ application-id }}' --required
