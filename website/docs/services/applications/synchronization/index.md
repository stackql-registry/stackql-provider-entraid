...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.applications.synchronization" /></td></tr>
</tbody></table>
...
secrets,
templates
FROM entraid.applications.synchronization
...
```sql
REPLACE entraid.applications.synchronization
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.applications.synchronization
WHERE application-id = '{{ application-id }}' --required
