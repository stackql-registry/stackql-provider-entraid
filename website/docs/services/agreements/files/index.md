...
<tr><td><b>Id</b></td><td><CopyableCode code="entra_id.agreements.files" /></td></tr>
</tbody></table>
...
language,
versions
FROM entra_id.agreements.files
...
```sql
INSERT INTO entra_id.agreements.files (
id,
@odata.type,
createdDateTime,
displayName,
...
```sql
UPDATE entra_id.agreements.files
SET 
id = '{{ id }}',
@odata.type = '{{ @odata.type }}',
createdDateTime = '{{ createdDateTime }}',
...
```sql
DELETE FROM entra_id.agreements.files
WHERE agreement-id = '{{ agreement-id }}' --required
AND agreementFileLocalization-id = '{{ agreementFileLocalization-id }}' --required
