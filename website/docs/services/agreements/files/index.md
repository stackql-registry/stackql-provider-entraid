...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.agreements.files" /></td></tr>
</tbody></table>
...
```sql
INSERT INTO entraid.agreements.files (
id,
@odata.type,
createdDateTime,
displayName,
...
```sql
UPDATE entraid.agreements.files
SET 
id = '{{ id }}',
@odata.type = '{{ @odata.type }}',
createdDateTime = '{{ createdDateTime }}',
...
```sql
DELETE FROM entraid.agreements.files
WHERE agreement-id = '{{ agreement-id }}' --required
AND agreementFileLocalization-id = '{{ agreementFileLocalization-id }}' --required
