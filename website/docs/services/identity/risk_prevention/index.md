...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.identity.risk_prevention" /></td></tr>
</tbody></table>
...
FROM entraid.identity.risk_prevention
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.identity.risk_prevention
SET 
...
```sql
DELETE FROM entraid.identity.risk_prevention
