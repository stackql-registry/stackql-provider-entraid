...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.identity.b2x_user_flows_languages" /></td></tr>
</tbody></table>
...
isEnabled,
overridesPages
FROM entraid.identity.b2x_user_flows_languages
...
```sql
INSERT INTO entraid.identity.b2x_user_flows_languages (
id,
@odata.type,
displayName,
isEnabled,
...
```sql
UPDATE entraid.identity.b2x_user_flows_languages
SET 
id = '{{ id }}',
@odata.type = '{{ @odata.type }}',
...
```sql
DELETE FROM entraid.identity.b2x_user_flows_languages
WHERE b2xIdentityUserFlow-id = '{{ b2xIdentityUserFlow-id }}' --required
AND userFlowLanguageConfiguration-id = '{{ userFlowLanguageConfiguration-id }}' --required
