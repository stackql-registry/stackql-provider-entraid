...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.tenant_relationships.multi_tenant_organization_join_request" /></td></tr>
</tbody></table>
...
role,
transitionDetails
FROM entraid.tenant_relationships.multi_tenant_organization_join_request
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.tenant_relationships.multi_tenant_organization_join_request
SET 
id = '{{ id }}',
