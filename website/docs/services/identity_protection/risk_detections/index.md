...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.identity_protection.risk_detections" /></td></tr>
</tbody></table>
...
userId,
userPrincipalName
FROM entraid.identity_protection.risk_detections
...
```sql
INSERT INTO entraid.identity_protection.risk_detections (
id,
@odata.type,
activity,
activityDateTime,
...
```sql
UPDATE entraid.identity_protection.risk_detections
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.identity_protection.risk_detections
WHERE riskDetection-id = '{{ riskDetection-id }}' --required
