...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.policies.cross_tenant_access_policy" /></td></tr>
</tbody></table>
...
partners,
templates
FROM entraid.policies.cross_tenant_access_policy
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.policies.cross_tenant_access_policy
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.policies.cross_tenant_access_policy
