...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.policies.authentication_methods_policy" /></td></tr>
</tbody></table>
...
FROM entraid.policies.authentication_methods_policy
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.policies.authentication_methods_policy
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.policies.authentication_methods_policy
