...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.policies.cross_tenant_access_policy_default" /></td></tr>
</tbody></table>
...
tenantRestrictions
FROM entraid.policies.cross_tenant_access_policy_default
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.policies.cross_tenant_access_policy_default
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.policies.cross_tenant_access_policy_default
...
```sql
EXEC entraid.policies.cross_tenant_access_policy_default.reset_to_system_default 
