...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.policies.admin_consent_request_policy" /></td></tr>
</tbody></table>
...
reviewers,
version
FROM entraid.policies.admin_consent_request_policy
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.policies.admin_consent_request_policy
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.policies.admin_consent_request_policy
