...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.policies.feature_rollout_policies" /></td></tr>
</tbody></table>
...
isEnabled
FROM entraid.policies.feature_rollout_policies
...
```sql
INSERT INTO entraid.policies.feature_rollout_policies (
id,
@odata.type,
description,
displayName,
...
```sql
UPDATE entraid.policies.feature_rollout_policies
SET 
id = '{{ id }}',
@odata.type = '{{ @odata.type }}',
...
```sql
DELETE FROM entraid.policies.feature_rollout_policies
WHERE featureRolloutPolicy-id = '{{ featureRolloutPolicy-id }}' --required
