...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.service_principals.synchronization" /></td></tr>
</tbody></table>
...
secrets,
templates
FROM entraid.service_principals.synchronization
...
```sql
REPLACE entraid.service_principals.synchronization
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.service_principals.synchronization
WHERE servicePrincipal-id = '{{ servicePrincipal-id }}' --required
