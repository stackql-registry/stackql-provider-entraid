...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.service_principals.synchronization_jobs_schema" /></td></tr>
</tbody></table>
...
version
FROM entraid.service_principals.synchronization_jobs_schema
...
```sql
UPDATE entraid.service_principals.synchronization_jobs_schema
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.service_principals.synchronization_jobs_schema
WHERE servicePrincipal-id = '{{ servicePrincipal-id }}' --required
AND synchronizationJob-id = '{{ synchronizationJob-id }}' --required
