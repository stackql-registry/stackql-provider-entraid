...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.service_principals.synchronization_jobs_schema_directories" /></td></tr>
</tbody></table>
...
readOnly,
version
FROM entraid.service_principals.synchronization_jobs_schema_directories
...
```sql
INSERT INTO entraid.service_principals.synchronization_jobs_schema_directories (
id,
@odata.type,
discoverabilities,
discoveryDateTime,
...
```sql
UPDATE entraid.service_principals.synchronization_jobs_schema_directories
SET 
id = '{{ id }}',
@odata.type = '{{ @odata.type }}',
...
```sql
DELETE FROM entraid.service_principals.synchronization_jobs_schema_directories
WHERE servicePrincipal-id = '{{ servicePrincipal-id }}' --required
AND synchronizationJob-id = '{{ synchronizationJob-id }}' --required
AND directoryDefinition-id = '{{ directoryDefinition-id }}' --required
