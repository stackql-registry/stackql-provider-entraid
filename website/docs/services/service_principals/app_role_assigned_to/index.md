...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.service_principals.app_role_assigned_to" /></td></tr>
</tbody></table>
...
resourceDisplayName,
resourceId
FROM entraid.service_principals.app_role_assigned_to
...
```sql
INSERT INTO entraid.service_principals.app_role_assigned_to (
id,
@odata.type,
deletedDateTime,
appRoleId,
...
```sql
UPDATE entraid.service_principals.app_role_assigned_to
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.service_principals.app_role_assigned_to
WHERE servicePrincipal-id = '{{ servicePrincipal-id }}' --required
AND appRoleAssignment-id = '{{ appRoleAssignment-id }}' --required
