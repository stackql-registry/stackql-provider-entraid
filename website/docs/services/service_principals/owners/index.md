...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.service_principals.owners" /></td></tr>
</tbody></table>
...
<tr>
    ...
    <td><a href="#parameter-servicePrincipal-id"><code>servicePrincipal-id</code></a></td>
    <td></td>
    <td>Use this API to add an owner for the servicePrincipal. Service principal owners can be users, the service principal itself, or other service principals.</td>
</tr>
<tr>
    ...
    <td>Remove an owner from a servicePrincipal object. As a recommended best practice, service principals should have at least two owners.</td>
</tr>
<tr>
...
@odata.type,
deletedDateTime
FROM entraid.service_principals.owners
...
```sql
INSERT INTO entraid.service_principals.owners (
@odata.id,
servicePrincipal-id
)
SELECT 
...
```sql
DELETE FROM entraid.service_principals.owners
WHERE servicePrincipal-id = '{{ servicePrincipal-id }}' --required
