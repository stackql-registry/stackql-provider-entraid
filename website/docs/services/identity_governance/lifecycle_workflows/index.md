...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.identity_governance.lifecycle_workflows" /></td></tr>
</tbody></table>
...
workflowTemplates,
workflows
FROM entraid.identity_governance.lifecycle_workflows
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.identity_governance.lifecycle_workflows
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.identity_governance.lifecycle_workflows
