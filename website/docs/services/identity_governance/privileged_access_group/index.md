...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.identity_governance.privileged_access_group" /></td></tr>
</tbody></table>
...
FROM entraid.identity_governance.privileged_access_group
WHERE $select = '{{ $select }}'
AND $expand = '{{ $expand }}'
;
```
...
```sql
UPDATE entraid.identity_governance.privileged_access_group
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.identity_governance.privileged_access_group
