...
<tr><td><b>Id</b></td><td><CopyableCode code="entraid.domains.service_configuration_records" /></td></tr>
</tbody></table>
...
supportedService,
ttl
FROM entraid.domains.service_configuration_records
...
```sql
INSERT INTO entraid.domains.service_configuration_records (
id,
@odata.type,
isOptional,
label,
...
```sql
UPDATE entraid.domains.service_configuration_records
SET 
id = '{{ id }}',
...
```sql
DELETE FROM entraid.domains.service_configuration_records
WHERE domain-id = '{{ domain-id }}' --required
AND domainDnsRecord-id = '{{ domainDnsRecord-id }}' --required
