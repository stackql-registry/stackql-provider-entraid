...
Creates, updates, deletes, gets or lists a <code>rejected_senders</code> resource.
...
<tr>
    ...
    <td>Users in the rejected senders list can't post to conversations of the group (identified in the GET request URL). Make sure you don't specify the same user or group in the rejected senders and accepted senders lists, otherwise you get an error.</td>
</tr>
<tr>
...
- name: rejected_senders
  props:
    - name: group-id
      value: "{{ group-id }}"
      description: Required parameter for the rejected_senders resource.
    - name: @odata.id
      value: "{{ @odata.id }}"
`}</CodeBlock>
