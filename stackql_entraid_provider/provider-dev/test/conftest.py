...
# Two parents above this file: the provider root (stackql_entraid_provider/).
# Anchor for resolving provider.yaml's `registry_root`. Named REPO_ROOT for
# backwards-compatibility with the archetype - this is NOT the git repo root.
...
def test_env(provider_config: ProviderConfig) -> dict[str, str]:
    ...
    if missing:
        pytest.exit(
            ...
            f"Source stackql_entraid_provider/provider-dev/.env (or set them) before running tests.",
            returncode=2,
        )
...
def stackql_binary() -> Path:
    if not BINARY.exists():
        pytest.exit(
            ...
            f"Run `bash provider-dev/test/bootstrap.sh` from stackql_entraid_provider/ first.",
            returncode=2,
        )
    return BINARY
